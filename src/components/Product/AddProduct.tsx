...
import AddProductMutation from '../../../GraphQl/mutation/addProduct'
...
const AddProduct = (props: AddProductProps) => {
  const [addProduct] = useMutation<AddProductData, AddProductParams>(
    AddProductMutation
  )
  const handleAddProduct = () => {
    addProduct({
      variables: { name: 'Test Product', description: 'This is a test product' }
    })
      .then(data => {
        console.log(data)
  ...
  const icon = (
    <Icon
      type={props.isLoading ? 'loading' : 'plus-circle'}
      style={{ fontSize: '64px' }}
