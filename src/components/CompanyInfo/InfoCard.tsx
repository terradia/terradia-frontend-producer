...
const InfoCard = (props: InfoCardProps) => {
  ...
  const onSubmit = (values) => {
    let newValues: info[] = rawInfo.infos;
    let i = 0;

    setIsSubmitting(false);
    setIsEditing(false);
    for (let key in values) {
      if (values.hasOwnProperty(key) && values[key] !== undefined && typeof values[key] === 'string')
        newValues[i] = { label: key, text: values[key] };
      else if (values.hasOwnProperty(key) && values[key] !== undefined && typeof values[key] !== 'string') {
